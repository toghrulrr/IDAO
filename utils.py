...
def load_train_hdf(path):
    return pd.concat([
        pd.read_hdf(os.path.join(path, "../mas/train_part_%i_%s.hdf" % (i, VERSION)))
        for i in (1, 2)], axis=0, ignore_index=True)

def load_private_test_hdf():
    return pd.read_hdf("../mas/test_private_v2_track_1.hdf")

def load_public_test_hdf():
    return pd.read_hdf("../mas/test_public_v2.hdf")
